...
> **Ghi chú:** File này mới được tạo khung (mục 1 đã điền sẵn theo `report/group_report.md` và `report/phan-cong-5-nguoi.md`). Bạn Trần Đức Bảo Trung cần tự điền các mục 2–10 bên dưới theo đúng phần việc, mức hiểu và kết quả thật của mình — không copy từ báo cáo của người khác.
...
> Gợi ý riêng cho Role 2: đây cũng là chỗ hợp lý để ghi lại vì sao commit code cho `cleaning.py`/`testset.py` lại nằm trong commit của Vi Minh Hiển thay vì tài khoản của bạn (môi trường chạy lúc đó chưa cài được trên máy bạn) — xem thêm mục 11 của `group_report.md`.
